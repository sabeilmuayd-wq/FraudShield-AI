...
translations = {
    "en": {
        ...
        "time_alert": "🕒 Unusual transaction time: {time} (late night)",
    ...
    "sw": {
        ...
        "time_alert": "🕒 Muda usio wa kawaida: {time} (usiku)",
...
class FraudDetector:
    def __init__(self):
        ...
        # ساعات متأخرة (11 مساءً حتى 4 صباحاً)
        self.late_hours = [23, 0, 1, 2, 3, 4]
        
    def is_late_hour(self, hour):
        """التحقق مما إذا كانت الساعة ضمن الساعات المتأخرة"""
        return hour in self.late_hours
    ...
    def calculate_risk(self, amount, location, device_type, time_str):
        ...
        # تحليل الوقت (مقارنة الساعة فقط)
        try:
            # محاولة استخراج الساعة من النص (متوقع تنسيق HH:MM)
            if ":" in time_str:
                hour = int(time_str.split(":")[0])
            else:
                hour = int(time_str)
            
            if self.is_late_hour(hour):
                risk += 20
                alerts.append(t("time_alert").format(time=time_str))
        except:
            # إذا فشل التحويل، تجاهل تحليل الوقت
            pass
...
<style>
    ...
    .alert-safe { background: #e8f5e9; border-left: 5px solid #2ecc71; padding: 1rem; border-radius: 10px; margin: 1rem 0; }
...
with tabs[3]:
    ...
    if not txns.empty:
        col1, col2 = st.columns(2)
        with col1:
            risk_counts = txns['risk_score'].value_counts().sort_index()
            fig = px.bar(x=risk_counts.index, y=risk_counts.values, title=t("transactions_by_risk"))
            st.plotly_chart(fig, use_container_width=True)
        with col2:
            loc_counts = txns['location'].value_counts()
            fig = px.pie(values=loc_counts.values, names=loc_counts.index, title=t("transactions_by_location"))
            st.plotly_chart(fig, use_container_width=True)
